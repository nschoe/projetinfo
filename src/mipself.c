...
static void relocZone(MemZone *Zone,  MemZone *EnsZones) {
    unsigned int i, sym, type, V, P, A, S, AHL, AHI, ALO;
	...
	// il faut que le code binaire ait déjà été chargé
	...
	// nbToReloc contient désormais le nombre d'entrée de la table de relocation
	
	Elf32_Rel * p_relocinfo = (Elf32_Rel *) data->d_buf;
	// p_relocinfo est désormais un tableau, de taille nbToReloc elements.
	// Chaque element i du tableau est de type Elf32_Rel et contient 
	// une entrée de la table de relocation.
	// Le type Elf32_Rel est présenté dans le poly p36.
	...
	{
	    sym = ELF32_R_SYM((p_relocinfo + i)->r_info);
	    type = ELF32_R_TYPE((p_relocinfo + i)->r_info);
	    P = (p_relocinfo + i)->r_offset;
	    S = (unsigned int) (EnsZones[sym - 1]).mem_start;
	    A = *(sectionareloger->data + P);
	    A = (A << 8) + *(sectionareloger->data + P + 1);
	    A = (A << 8) + *(sectionareloger->data + P + 2);
	    A = (A << 8) + *(sectionareloger->data + P + 3);
	    switch(type)
	    {
	    case R_MIPS_32 :
		V = S + A;
	    ...
	    case R_MIPS_HI16 :
		AHI = (short)A;
		ALO = *(sectionareloger->data + P + 4 + 2);
		ALO = (ALO << 8) + *(sectionareloger->data + P + 4 + 3);
		AHL = (AHI << 16) + (short)ALO;
		V = A | ((AHL + S - ((short)AHL + S)) >> 16);
		break;
	    case R_MIPS_LO16 :
		AHI = *(sectionareloger->data + P - 4 + 2);
		AHI = (AHI << 8) + *(sectionareloger->data + P - 4 + 3);
		ALO = (short)A;
		AHL = (AHI << 16) + (short)ALO;
		V = A | (short)(AHL + S);
		break;
	    default :
		ERROR_MSG("Relocation type %d is not implemented", type);
	    }
	    *(sectionareloger->data + P) = (char)(V >> 24);
	    *(sectionareloger->data + P + 1) = (char)(V >> 16);
	    *(sectionareloger->data + P + 2) = (char)(V >> 8);
	    *(sectionareloger->data + P + 3) = (char)V;
	}
    }
}
